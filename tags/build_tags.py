...
# --------------------------------------------------------------------------
# the tags, embedded in the project
# --------------------------------------------------------------------------

TAGDATA = os.path.join(
    os.path.dirname(HERE), "project", "ignition", "script-python",
    "AlarmDemo", "tagdata", "code.py")
...
across many lines only because a %d-character single line is unreadable in a
...
def write_tagdata(export):
    """Write project/.../AlarmDemo/tagdata/code.py from the same export."""
    text = json.dumps(export, separators=(",", ":"))
    d = os.path.dirname(TAGDATA)
    if not os.path.isdir(d):
        os.makedirs(d)
    width = 96
    chunks = [text[i:i + width] for i in range(0, len(text), width)]
    body = "".join("    %s\n" % json.dumps(c) for c in chunks)
    with open(TAGDATA, "w") as f:
        f.write(TAGDATA_HEADER % len(text))
        f.write(body)
        f.write(")\n")
    print("tagdata: %d bytes in %d lines" % (len(text), len(chunks)))
...
def main():
    ...
    # Designer-importable export of the whole provider, for anyone who would
    # rather import the tags by hand than press Create on the Setup screen.
    # It is not part of the install any more - the project carries its own
    # copy (see write_tagdata below) - but it is the file to hand someone who
    # asks "what are the tags".
    export = {
        "name": "",
        "tagType": "Provider",
    ...
    # The same export, embedded in a project script resource.
    #
    # This is what makes the demo a STANDALONE import: a project export carries
    # project resources and nothing else, so the tags had to travel some other
    # way. They used to travel as a second file in an Exchange package that a
    # human imported by hand in the Designer. Now the project carries them and
    # AlarmDemo.setup writes them into the provider with system.tag.configure,
    # so importing the project is the whole install.
    write_tagdata(export)
