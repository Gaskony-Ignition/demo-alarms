...
- **It will not overwrite a database connection.** `createDatabase` refuses
  ...
  connection turns out to be the wrong engine, `_databaseFix` makes a new
  SQLite one under a free name (`AlarmDemoDB_SQLite`) and repoints the demo —
  the other connection is not touched.
