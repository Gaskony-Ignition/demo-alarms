...
**Nothing is manual any more.** The `AlarmDemo` tag provider and the alarm
journal profile were the last two exceptions — gateway config resources, so
they could not be in a project export, and they used to ship as files under a
`gateway/` directory that a human applied with a config scan. `system.config`
creates both from the project now, live, with no scan and no restart, and that
directory is gone. Every item on the Setup screen has a button.
